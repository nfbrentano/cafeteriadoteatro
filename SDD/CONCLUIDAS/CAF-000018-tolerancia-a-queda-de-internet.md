...
- [x] Indicador online/offline no PDV e na cozinha (eventos `online` / `offline` + estado do canal realtime).
- [x] PDV offline: o pedido vai para uma fila local (IndexedDB) com status "Aguardando envio".
- [x] Envio automático da fila quando a conexão voltar, em ordem.
- [x] Cada pedido tem um id gerado no aparelho (`client_id UUID`), e `criar_pedido` ignora duplicados (idempotência).
- [x] Lista "Pedidos pendentes de envio" com opção de reenviar ou descartar.
- [x] Cozinha: faixa vermelha "Sem conexão — pedidos novos podem não aparecer" quando offline.
- [x] Cardápio do PDV (produtos, adicionais, promoções) disponível em cache para montar pedidos offline.

### Requisitos não funcionais
- [x] Nenhum pedido enviado duas vezes, mesmo com várias tentativas.
- [x] Preço final continua calculado no banco no momento do envio; o carrinho offline mostra "valor estimado".
- [x] Fila local sobrevive a fechar e reabrir o app.
...
- [x] **Dado que** o PDV está offline, **quando** o barista envia um pedido, **então** ele fica na fila local com status "Aguardando envio".
- [x] **Dado que** há 2 pedidos na fila, **quando** a conexão volta, **então** eles são enviados em ordem e aparecem na cozinha.
- [x] **Dado que** o envio falhou depois de o banco já ter gravado, **quando** o PDV tenta de novo, **então** o pedido não é duplicado.
- [x] **Dado que** a cozinha perdeu a conexão, **quando** isso acontece, **então** a faixa vermelha aparece em até 5 s.
- [x] **Dado que** o app foi fechado com pedidos na fila, **quando** é reaberto online, **então** a fila é enviada.
